...
class block_course_files extends block_base {
    public function init() {
        $this->title = get_string('pluginname', 'block_course_files');
    }
    ...
    public function get_content() {
        ...
        if (!$course_path) {
            $this->content->text .= get_string('nofiles', 'block_course_files');
            return $this->content;
        }
        ...
        $this->content->text .= '<p><i>Assistent de intel·ligència artificial</i></p>';
        $this->content->text .= '<form id="myForm">';
        $this->content->text .= '<label for="user_input">Introdueix la pregunta:</label>';
        ...
        $this->content->text .= '<input type="submit" value="Envia" />';
        ...
        <script>
            document.getElementById('myForm').addEventListener('submit', function(event) {
                ...
                if (!userInput) {  
                    alert('Si us plau, ingresseu un text vàlid.'); 
                    return;
                }
                ...
                .then(data => {
                    document.getElementById('result').innerText = 'API Response: ' + data.response;
                })
                .catch(error => {
